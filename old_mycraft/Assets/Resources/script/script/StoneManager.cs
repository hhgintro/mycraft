using System.Collections;
using System.Collections.Generic;
using UnityEngine;

namespace MyCraft
{
    public class StoneManager : BlockManager
    {

        //private List<DrillScript> prefabs_drill;

        // Use this for initialization
        protected override void Start()
        {
            base.Start();

            //this.prefabs_drill = new List<DrillScript>();
            //this.prefabs.Add(this.transform.Find("prefab/stone").GetComponent<StoneScript>());
            BlockScript script = this.transform.Find("prefab/stone").GetComponent<StoneScript>();
            script.manager = this;
            this.prefabs.Add(script);

        }

        //// Update is called once per frame
        //void Update()
        //{

        //}

        //public override BlockScript GetChoicePrefab(TURN_WEIGHT weight)
        //{
        //    if (this.prefabs_drill.Count <= 0)
        //        return null;

        //    BlockScript prefab = this.prefabs_drill[0];
        //    if (null == prefab) return null;
        //    prefab.GetComponent<Collider>().enabled = false;
        //    return prefab;
        //}

        //public override void CreateBlock(BlockScript script)
        //{
        //    if (null == script || BLOCKTYPE.DRILL != script.blocktype)
        //        return;

        //    ((DrillScript)script).running = true;
        //}
    }


}