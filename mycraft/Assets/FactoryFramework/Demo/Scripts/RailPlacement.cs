...
public class RailPlacement : IPlacement
{
	//
	GameObject _prefabLoop;

	GameObject _prefabLoopNode;
	SplineComputer _loop;   //생성할 Loop
	int _startIndex = 0;
	int _endIndex = 0;

	private Rail current;
	...
	{
		_prefabLoop = MyCraft.Managers.Resource.Load<GameObject>("prefabs/Splines/Junction/Loop");
		_prefabLoopNode = MyCraft.Managers.Resource.Load<GameObject>("prefabs/Splines/Junction/LoopNode");
	}

	//OnDestroy에서 호출된다.
	//RailPlacement에서 Pool_Rail를 먼저 정리하고(belt의 아이템을 날리기 위해)
	//BuildingPlacement에서 나머지 Pool_xxx을 정리하게 됩니다.
	protected override void OnfnDestroy()
	{
		Debug.Log("RailPlacement 종료를 진행합니다.");
		//Debug.Log($"-  {this.name}");
		Transform go = this.transform.Find("Pool_Rail");
		if(null != go)
		{
			//Debug.Log($"+  {go.name}");
			...
			for (int i = go.childCount - 1; i >= 0; --i)
			{
				...
				Rail rail = go.GetChild(i).GetComponent<Rail>();
				if (null == rail) continue;
				
				rail.OnDeleted(false); //false:밸트위 아이템을 인벤으로 회수하지 않고 날린다.
				MyCraft.Managers.Resource.Destroy(rail.gameObject);
			}
		}
	}
	...
	{
		Debug.Log("ForceCancel RailPlacement");
		if (current != null)
		{
		...
		if(null != _loop) _loop.GetComponent<MeshCollider>().enabled = true;	//@@3
		_loop = null;
		current = null;
		startSocket = null;
		endSocket = null;
	...
	public GameObject StartPlacingLoop(GameObject prefab)
	{
		////cancel any placement currently happening
		//cancelPlacementEvent?.Raise();
		// instantiate a belt to place
		_loop = MyCraft.Managers.Resource.Instantiate(prefab).GetComponent<SplineComputer>();    //HG[2023.06.01]테스트필요
		//_loop.transform.parent = this.transform.Find($"Pool_{prefab.name}") ?? (new GameObject($"Pool_{prefab.name}") { transform = { parent = this.transform } }).transform;
		_loop.transform.parent = MyCraft.Common.ParentPool(this.transform, prefab.name);
		...
		_loop.GetComponent<MeshCollider>().enabled = false;

		//HG_TEST
		//
		//_loop.GetComponent<MeshRenderer>().sharedMaterial = greenGhostMat;
		return _loop.gameObject;
	}

	public GameObject StartPlacingRail(GameObject prefab)
	{
		//HG_TEST:임시로 loop를 만들어 줍니다.
		StartPlacingLoop(_prefabLoop);
		_startIndex = AddPoint_1(_loop, new Vector3(0f, 0f, 0f));    //시작점 생성	//@@3
		_endIndex = AddPoint_1(_loop, Vector3.forward*10);	//종료점 생성

		//cancel any placement currently happening
		cancelPlacementEvent?.Raise();
		// instantiate a belt to place
		current = MyCraft.Managers.Resource.Instantiate(prefab).GetComponent<Rail>();    //HG[2023.06.01]테스트필요
		...
		current.transform.position += Vector3.up*3;	//HG_TEST:
		// trigger event
		startPlacementEvent?.Raise();
	...
	int AddPoint_1(SplineComputer sourceComputer, Vector3 point, int sourceIndex = -1, SplineComputer destComputer = null, int destIndex = -1)
	{
		...
		SplinePoint spline = sourceComputer.AddPointPosition_1(sourceIndex, point);//삽입
	...
	protected override void HandleStartState()
	{
		...
		if (null == _loop) StartPlacingLoop(_prefabLoop);   //loop가 없으면 먼저 만들어 줍니다.		//@@3
		...
		_loop.SetPointPosition(_startIndex, worldPos);  //설치시작	//@@3
		_loop.SetPointPosition(_endIndex, worldPos + worldDir*10);
		...
		{
			_loop.SetPointPosition(_startIndex, worldPos);	//시작점 고정	//@@3
			base.TryChangeState(State.End);
		}
	}
	//conveyor 끝부분 지정할 때
...
;                   }
		...
		//current.data.endDir = worldDir;
		_loop.SetPointPosition(_endIndex, worldPos);	//종료위치 찾기		//@@3
		...
		if (Input.GetMouseButtonDown(0) && ValidMesh)
		{
			...
			_loop.SetPointPosition(_endIndex, worldPos);   //종료점 고정		//@@3
			_endIndex = AddPoint_1(_loop, worldPos + Vector3.forward*10);  //종료점 다시 생성
		}
	}
}
