...
namespace MyCraft
{
	...
	class Common
	{
		...
		public static Vector3 Floor(Vector3 pos)
		{
			...
			float y = pos.y;    //Mathf.Floor(pos.y + 0.5f);	//발판에 뭍혀 생성할 수 없는 경우가 생긴다.
			float z = Mathf.Floor(pos.z + 0.5f);
			return new Vector3(x, y, z);
		}

		public static Transform ParentPool(Transform owner, string name)
		{
			//owner하위에 "Pool_xxx"을 생성하고 그 하위에 prefab을 하위에 둔다.
			return owner.Find($"Pool_{name}") ?? (new GameObject($"Pool_{name}") { transform = { parent = owner } }).transform;
		}
	}//..class Common
