...
namespace FactoryFramework
{
    ...
    public class TempMesh
    {
        ...
        public int GetIndexOfPosNormUV(Vector3 pos, Vector3 norm, Vector2 uv)
        {
            for (int i = 0; i < vertices.Count; i++)
            {
                if (vertices[i].pos == pos && vertices[i].normal == norm && vertices[i].uv0 == uv)
                    return i;
            }
            return -1;
        }
        public void AddRawTriangle(LinkedVertex[] lva)
        {
            ...
            {
                var lv = lva[i];
                int index = GetIndexOfPosNormUV(lv.pos, lv.normal,lv.uv0);
                if (index == -1)
                {
                    vertices.Add(lv);
        ...
        public void AddTriangleCopyFromMesh(Mesh m, int[] indices)
        {
            for (int i = 0; i < indices.Length; i++)
            {
                ...
                int localvertindex = GetIndexOfPosNormUV(v.pos, v.normal, v.uv0);
                if (localvertindex == -1)
                {
                    vertices.Add(v);
