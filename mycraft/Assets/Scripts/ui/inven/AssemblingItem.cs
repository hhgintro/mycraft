...
namespace MyCraft
{
    public class AssemblingItem
    {
        ...
        public List<int> inputs = new List<int>(); //투입가능한 itemid
        ...
        private void LoadInputs(JsonData json)
        {
            if (false == json.Keys.Contains("inputs"))
                return;

            //생산품 개수만 표기한 경우(자신의 itemid를 생산한다.)
            if (false == json["inputs"].IsArray)
            {
                inputs.Add((int)json["inputs"]);
                return;
            }

            //생산품을 지정한 경우(배열일때 - 연구소)
            for (int i = 0; i < json["inputs"].Count; ++i)
                inputs.Add((int)json["inputs"][i]); //투입가능한 아이템 id
        }
    }
