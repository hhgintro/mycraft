namespace Dreamteck.Splines
{
    ...
    public partial class SplineComputer : MonoBehaviour
    {
		...
		//The sample rate of the spline – the bigger, the smoother
        ...
        private void RunUpdate(bool immediate = false)
        {
            ...
            if (_queueResample)
            {
                ...
                {
                    CalculateSamples(!transformChanged);//@@2-4
				}
            }
        ...
        public void SetPointPosition(int index, Vector3 pos, Space setSpace = Space.World)
        {
            ...
            if (newPos != _spline.points[index].position)
            {
                ...
                _spline.points[index].normal = Vector3.up;
		...
		//add new point
		//기존 위치를 변경코자 할때는 SetPointPosition()에서 처리합니다.(우선 테스트는 못함)
		public SplinePoint AddPointPosition_1(int index, Vector3 newPoint, Space setSpace = Space.World)
		{
			ResampleTransformIfNeeded();
			if (index < 0 || _spline.points.Length < index) index = _spline.points.Length;  //bad index는 맨끝에 넣어준다.

            AppendPoints_1(index);

			Vector3 newPos = newPoint;
			if (_space == Space.Local && setSpace == Space.World) newPos = InverseTransformPoint(newPoint);
		...
		public SplinePoint[] AppendPoints_1(int index)
        {
			...
			{
				if (index == i) continue;//이 자리는 신규point로 채울예정이므로 비워둔다.
        ...
        private void TransformSamples()
        {
            ...
            } else
            {
                for (int i = 0; i < _rawSamples.Length; i++)
                {
                    ...
                    {
                        TransformSample(ref _sampleCollection.samples[i]);//@@2-6
                    }
                }
            }
        }
